//
//  MangaBoxNotification.h
//  MangaBox
//
//  Created by Ken Tran on 10/3/14.
//  Copyright (c) 2014 Ken Tran. All rights reserved.
//

#ifndef MangaBox_MangaBoxNotification_h
#define MangaBox_MangaBoxNotification_h

#define addingNewMangaToCollectionNotification @"addingNewMangaToCollectionNotification"

#define startAdvancedSearchNotification @"startAdvancedSearchNotification"

#define finishDownloadChapterPage @"finishDownloadChapterPage"
#define errorDownloadingChapter @"errorDownloadingChapter"
#define finishDownloadChapter @"finishDownloadChapter"

#define autoNextChapterNotification @"autoNextChapterNotification"
#define autoPreviousChapterNotification @"autoPreviousChapterNotification"

#define parsedMangaSelected @"parsedMangaSelected"

#endif